...
const ROUTE_STYLE = {
  forwardLineWeight: 8,
  reverseLineWeight: 6,
  forwardSegmentHaloWeight: 30,
  reverseSegmentHaloWeight: 26,
  forwardSegmentWeight: 22,
  reverseSegmentWeight: 18
};
const SETTINGS_KEYS = [
  "poll_seconds",
  "sample_limit",
...
function showCameraHoverPreview(cam, evt) {
  ...
  // Disable hover card only on true touch/coarse-pointer devices.
...
function renderRouteMap(payload) {
  ...
  if (forward?.route_available && forward.route_line_geojson) {
    const forwardLineColor = forwardSegCount > 0 ? "#7dd3fc" : "#fbbf24";
    routeForwardLineLayer = window.L.geoJSON(forward.route_line_geojson, {
      style: { color: forwardLineColor, weight: ROUTE_STYLE.forwardLineWeight, opacity: 0.95 }
    }).addTo(routeMap);
  ...
  if (reverse?.route_available && reverse.route_line_geojson) {
    const reverseLineColor = reverseSegCount > 0 ? "#f9a8d4" : "#f59e0b";
    routeReverseLineLayer = window.L.geoJSON(reverse.route_line_geojson, {
      style: { color: reverseLineColor, weight: ROUTE_STYLE.reverseLineWeight, opacity: 0.95, dashArray: "10,8" }
    }).addTo(routeMap);
