...
let routeMap = null;
let routeScaleControl = null;
let routeForwardSegmentsLayer = null;
let routeReverseSegmentsLayer = null;
let routeForwardSegmentsHaloLayer = null;
let routeReverseSegmentsHaloLayer = null;
...
const ROUTE_STYLE = {
  forwardLineWeight: 24,
  reverseLineWeight: 20,
  forwardSegmentHaloWeight: 62,
  reverseSegmentHaloWeight: 54,
  forwardSegmentWeight: 46,
  reverseSegmentWeight: 38
};
const SETTINGS_KEYS = [
  "poll_seconds",
  "sample_limit",
...
function updateLastPointerPos(evt) {
...
function cameraHoverAnchorPos(anchorEl) {
  const r = anchorEl?.getBoundingClientRect?.();
  if (!r) return null;
  if (!Number.isFinite(r.left) || !Number.isFinite(r.top) || !Number.isFinite(r.width) || !Number.isFinite(r.height)) return null;
  return {
    x: r.left + Math.min(Math.max(r.width * 0.75, 40), Math.max(40, r.width - 10)),
    y: r.top + Math.max(20, Math.min(r.height * 0.5, r.height - 8))
  };
}

function cameraHoverPointerPos(evt, anchorEl) {
  const x = Number(evt?.clientX);
  const y = Number(evt?.clientY);
  if (Number.isFinite(x) && Number.isFinite(y) && (x !== 0 || y !== 0)) {
    lastPointerPos = { x, y };
  ...
  const anchorPos = cameraHoverAnchorPos(anchorEl);
  if (anchorPos) return anchorPos;
  if (Number.isFinite(lastPointerPos.x) && Number.isFinite(lastPointerPos.y) && (lastPointerPos.x !== 0 || lastPointerPos.y !== 0)) {
    return { x: lastPointerPos.x, y: lastPointerPos.y };
  }
  return {
    x: Math.round((window.innerWidth || 1200) * 0.6),
    y: Math.round((window.innerHeight || 900) * 0.45)
  };
}

function positionCameraHoverCard(pos) {
  ...
  let left = Number(pos.x || 0) + gap;
  let top = Number(pos.y || 0) - rect.height * 0.35;
  if (left + rect.width + pad > vw) left = Math.max(pad, Number(pos.x || 0) - rect.width - gap);
...
function showCameraHoverPreview(cam, evt, anchorEl = null) {
  ...
  const pos = cameraHoverPointerPos(evt, anchorEl);
  positionCameraHoverCard(pos);
}
...
function renderCameraRows(rows) {
  ...
  rows.forEach((r) => {
    ...
    tr.addEventListener("pointerenter", (e) => {
      updateLastPointerPos(e);
      setCameraPreview(r);
      showCameraHoverPreview(r, e, tr);
    });
    tr.addEventListener("pointermove", (e) => {
      updateLastPointerPos(e);
      showCameraHoverPreview(r, e, tr);
    });
    tr.addEventListener("pointerleave", hideCameraHoverPreview);
...
function routeNum(v, digits = 1) {
  const n = Number(v);
  return Number.isFinite(n) ? n.toFixed(digits) : "-";
}

function routeSegmentTooltipHtml(feature) {
  const p = feature?.properties || {};
  return (
    `Delay ${routeNum(p.delay_pct, 1)}% | ` +
    `Speed ${routeNum(p.speed_mph, 1)} / FF ${routeNum(p.speed_ff_mph, 1)} mph | ` +
    `Jam ${routeNum(p.jam_factor, 1)}`
  );
}
...
function ensureRouteMap() {
  ...
  routeScaleControl = window.L.control.scale({ imperial: true, metric: true, position: "bottomleft" });
  routeScaleControl.addTo(routeMap);
}

function resetRouteLayers() {
  [
    routeForwardSegmentsHaloLayer,
    routeReverseSegmentsHaloLayer,
    routeForwardSegmentsLayer,
    routeReverseSegmentsLayer,
    routeForwardLineLayer,
    routeReverseLineLayer
  ].forEach((layer) => {
    if (layer && routeMap) routeMap.removeLayer(layer);
  });
  routeForwardSegmentsHaloLayer = null;
  routeReverseSegmentsHaloLayer = null;
...
function renderRouteMap(payload) {
  ...
  const forward = payload.forward || null;
  const reverse = payload.reverse || null;
  const forwardSegCount = Number(forward?.stats?.segment_count || 0);
  const reverseSegCount = Number(reverse?.stats?.segment_count || 0);
  ...
  if (forward?.route_available && forward.route_line_geojson) {
    const forwardAvgDelay = Number(forward?.stats?.avg_delay_pct || 0);
    const forwardLineColor = forwardSegCount > 0 ? routeDelayColor(forwardAvgDelay) : "#fbbf24";
    routeForwardLineLayer = window.L.geoJSON(forward.route_line_geojson, {
      style: { color: forwardLineColor, weight: ROUTE_STYLE.forwardLineWeight, opacity: 0.98 }
    }).addTo(routeMap);
    routeForwardSegmentsHaloLayer = window.L.geoJSON(forward.segments_geojson, {
      style: () => ({
        color: "rgba(7, 16, 26, 0.88)",
        weight: ROUTE_STYLE.forwardSegmentHaloWeight,
        opacity: 0.9,
        lineCap: "round",
        lineJoin: "round"
      })
    ...
    routeForwardSegmentsLayer = window.L.geoJSON(forward.segments_geojson, {
      style: (f) => ({
        ...
        weight: ROUTE_STYLE.forwardSegmentWeight,
        opacity: 0.98,
        lineCap: "round",
        lineJoin: "round"
      }),
      onEachFeature: (feature, layer) => {
        layer.bindTooltip(routeSegmentTooltipHtml(feature), {
          sticky: true,
          opacity: 0.96,
          direction: "top"
        });
  ...
  if (reverse?.route_available && reverse.route_line_geojson) {
    const reverseAvgDelay = Number(reverse?.stats?.avg_delay_pct || 0);
    const reverseLineColor = reverseSegCount > 0 ? routeDelayColor(reverseAvgDelay) : "#f59e0b";
    routeReverseLineLayer = window.L.geoJSON(reverse.route_line_geojson, {
      style: { color: reverseLineColor, weight: ROUTE_STYLE.reverseLineWeight, opacity: 0.96, dashArray: "10,8" }
    }).addTo(routeMap);
    routeReverseSegmentsHaloLayer = window.L.geoJSON(reverse.segments_geojson, {
      style: () => ({
        color: "rgba(7, 16, 26, 0.76)",
        weight: ROUTE_STYLE.reverseSegmentHaloWeight,
        opacity: 0.8,
        dashArray: "10,8",
        lineCap: "round",
        lineJoin: "round"
    ...
    routeReverseSegmentsLayer = window.L.geoJSON(reverse.segments_geojson, {
      style: (f) => ({
        color: routeDelayColor(f?.properties?.delay_pct),
        weight: ROUTE_STYLE.reverseSegmentWeight,
        opacity: 0.94,
        dashArray: "10,8",
        lineCap: "round",
        lineJoin: "round"
  ...
  if (bounds.length) {
    ...
    routeMap.fitBounds(merged, { padding: [22, 22], maxZoom: 15 });
  } else {
  ...
  if (statusEl) {
    const segmentSummary = ` | segments drawn: TO=${forwardSegCount}${reverse ? `, FROM=${reverseSegCount}` : ""}`;
    const noSegmentsNotice =
      forwardSegCount + reverseSegCount === 0
        ? " | No route-scoped traffic segments found, so color bands are not available yet. Try Route buffer km 12-20, then Run Poll Now."
        : "";
    statusEl.textContent =
      `Forward=${forward?.corridor_name || "-"}${reverseLabel} | buffer=${Number(payload.route_buffer_km || 0).toFixed(
        1
      )} km | fetched ${new Date(payload.fetched_at || Date.now()).toLocaleTimeString()}${segmentSummary}${noSegmentsNotice}`;
  }
  routeForwardLineLayer?.bringToFront?.();
  routeReverseLineLayer?.bringToFront?.();
  routeForwardSegmentsLayer?.bringToFront?.();
  routeReverseSegmentsLayer?.bringToFront?.();
  renderDirectionCompare(payload);
}
