...
function renderCameraRows(rows) {
  ...
  rows.forEach((r) => {
    ...
    tr.addEventListener("mouseenter", () => setCameraPreview(r));
...
async function refreshAll() {
  ...
  try {
    const [latest, ts, cams, model, insights, weather, weights] = await Promise.all([
...
async function renderMermaidBlocks(blocks, holderId) {
  ...
  try {
    ...
    holder.innerHTML = `<div class="status">Diagram render failed: ${escapeHtml(e.message || e)}</div>`;
  }
}
...
async function init() {
  ...
  $("radarProductSelect").addEventListener("change", (e) => {
    ...
    applyRadarProductToLayer();
    applyRadarFrameToLayers();
    if (latestWeather) renderRadarMap(latestWeather);
  });
