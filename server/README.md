...
Server app for live corridor delay analytics using public statewide feeds (Illinois ArcGIS + California CWWP2).
...
- Add periodic backups:
 ...
 - Multi-profile ingestion:
  - Per-corridor feed profile selection (`il_arcgis` or `ca_cwwp2`)
  - CA supports CCTV + lane closures + chain controls + travel-time segments
  - Optional per-corridor CA district scoping via `feed_config.districts`
- Directional incident relevance:
  - `incident_relevance_mode=2` (strict directional) default
  - Modes: legacy bbox, relaxed near-route, strict directional
  - Route buffer + direction mismatch penalties are runtime-configurable
