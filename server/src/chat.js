...
async function latestStats(corridorId) {
  const { rows } = await pool.query(
    `SELECT run_ts, fused_score, predicted_next_score_p50, predicted_next_score_p90, prediction_confidence,
            incidents_count, closures_count, alert_state
       FROM monitor_runs
      WHERE corridor_id = $1
...
async function maybeAskOpenAI(question, context) {
  ...
  try {
        ...
        input: [
          {
            role: "system",
            content:
              "You are a transport analytics copilot. Use only provided context values. Be concise and actionable."
          },
          {
            role: "user",
    ...
    if (!res.ok) {
      ...
      return { text: null, error: `OpenAI HTTP ${res.status}: ${errText.slice(0, 400)}` };
    }
    const data = await res.json();
    return { text: data.output_text || null, error: null };
  } catch (e) {
    return { text: null, error: `OpenAI request failed: ${String(e.message || e)}` };
  }
}
...
export async function answerAnalyticsQuestion({ corridorId, question }) {
  ...
  if (containsAny(q, ["predict", "forecast", "next"])) {
    ...
    answer =
      ...
      `(alert=${latest.alert_state}, incidents=${latest.incidents_count}, closures=${latest.closures_count}). ` +
