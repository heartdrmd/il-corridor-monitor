...
async function latestRuns(corridorId, limit = 2) {
  const { rows } = await pool.query(
    `SELECT run_ts, fused_score, traffic_segments, delay_pct, incidents_count, closures_count, camera_total, camera_fresh_pct,
            image_score, traffic_component, image_component, incident_component, closure_component,
...
async function buildContext(corridorId) {
  const [corridor, settings, runs, series, trend24, trend72, hourly, model, checkpoints, hotspots, cameras] =
    await Promise.all([
...
async function maybeAskOpenAI(question, context) {
  ...
  const instructions =
    "You are a corridor operations analyst copilot. Use only provided context values, cite key numbers, and give practical next actions. If data is missing, say exactly what is missing.";
...
export async function answerAnalyticsQuestion({ corridorId, question }) {
  ...
  if (containsAny(q, ["predict", "forecast", "next"])) {
    ...
    answer =
      `Runtime settings loaded: weather_component_max=${Number(s.weather_component_max || 0)}, ` +
      `weight_traffic=${Number(s.weight_traffic || 0)}, weight_image_with_traffic=${Number(
        s.weight_image_with_traffic || 0
      )}, incident_saturation_max=${Number(s.incident_saturation_max || 0)}, ` +
      `alert_high_score_threshold=${Number(s.alert_high_score_threshold || 0)}.`;
