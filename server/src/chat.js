...
import { getActiveMethodologyPreset, getActiveStrategy, getRuntimeSettings } from "./settings.js";
...
function methodologySummary(settings = {}, corridor = null) {
  ...
  const relevanceCode = Number(settings.incident_relevance_mode ?? 2);
  return {
    score_formula:
    ...
    feed_profile: corridor?.feed_profile || "il_arcgis",
    incident_relevance_mode: relevanceModeLabel(relevanceCode),
    incident_route_buffer_km_relaxed: Number(settings.incident_route_buffer_km_relaxed ?? 12),
    incident_route_buffer_km_strict: Number(settings.incident_route_buffer_km_strict ?? 6),
    incident_relaxed_direction_penalty: Number(settings.incident_relaxed_direction_penalty ?? 0.45),
...
async function buildContext(corridorId) {
  ...
  return {
    ...
    active_methodology_preset: activeMethodologyPreset,
    source_feed_counts: feedCounts,
    runtime_settings: settings,
    methodology: methodologySummary(settings, corridor),
    context_coverage: [
      "corridor_meta",
      "latest_run+delta",
...
async function maybeAskOpenAI(question, context) {
  ...
  const instructions =
    "You are a corridor operations analyst copilot. Use only provided context values, cite key numbers, and give practical next actions. If data is missing, say exactly what is missing. When asked for visuals, include a concise Mermaid diagram in a ```mermaid fenced block plus a short interpretation. Mermaid rules: valid Mermaid v11 syntax only; one edge/node statement per line; no HTML tags like <br/>; quote labels with [] as A[\"text\"]; avoid percent symbols (write pct).";
...
export async function answerAnalyticsQuestion({ corridorId, question }) {
  ...
  if (containsAny(q, ["predict", "forecast", "next"])) {
    ...
    const preset = context.active_methodology_preset || null;
    const presetText = preset?.id ? `active_preset=${preset.id}` : "active_preset=none";
    answer =
      ...
      )}, incident_relevance_mode=${Number(s.incident_relevance_mode || 0)} (${context.methodology?.incident_relevance_mode || "legacy_bbox"}), ` +
      `feed_profile=${context.corridor?.feed_profile || "il_arcgis"}. ` +
      `${presetText}. ` +
