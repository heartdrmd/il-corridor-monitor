...
export async function runCorridorPoll(corridor, { sampleLimit = 40, baselineAlpha = 0.2 } = {}) {
  ...
  return withTx(async (client) => {
    ...
    const model = trainNextScoreModel(trainingRuns);
      ...
      [
        ...
        JSON.stringify({
          ...
          feedCounts: {
            ...
            cameras_scoped: cameraRows.length
          }
        })
      ]
    );
...
export async function runPollCycle(opts) {
  const { rows } = await pool.query(
    "SELECT id, name, xmin, ymin, xmax, ymax FROM corridors WHERE active = true ORDER BY id"
  );
  const out = [];
  for (const c of rows) {
    try {
      out.push(await runCorridorPoll(c, opts));
    } catch (e) {
