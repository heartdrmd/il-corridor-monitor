...
import { fetchNormalizedFeeds } from "./feedProviders.js";
...
function nearestRouteDistanceKm(point, routePoints) {
  ...
  for (const rp of routePoints) {
    const d = distanceKm(Number(point.lat), Number(point.lon), Number(rp.lat), Number(rp.lon));
    if (d < best) best = d;
  }
  return best;
}

function directionToken(raw) {
  const t = String(raw || "").toUpperCase();
  if (!t) return "";
  if (t.includes("NORTH") || t.includes("NB") || /\bN\b/.test(t)) return "N";
  if (t.includes("SOUTH") || t.includes("SB") || /\bS\b/.test(t)) return "S";
  if (t.includes("EAST") || t.includes("EB") || /\bE\b/.test(t)) return "E";
  if (t.includes("WEST") || t.includes("WB") || /\bW\b/.test(t)) return "W";
  return "";
}

function routeDirectionToken(routePoints = []) {
  if (!Array.isArray(routePoints) || routePoints.length < 2) return "";
  const first = routePoints[0];
  const last = routePoints[routePoints.length - 1];
  const dLat = Number(last.lat) - Number(first.lat);
  const dLon = Number(last.lon) - Number(first.lon);
  if (Math.abs(dLon) >= Math.abs(dLat)) return dLon >= 0 ? "E" : "W";
  return dLat >= 0 ? "N" : "S";
}

function featureDirectionText(attrs = {}) {
  const keys = [
    "Direction",
    "direction",
    "DIR",
  ...
  for (const k of keys) {
    if (typeof attrs?.[k] === "string" && attrs[k].trim()) return attrs[k].trim();
  }
  for (const [k, v] of Object.entries(attrs || {})) {
    if (/dir/i.test(k) && typeof v === "string" && v.trim()) return v.trim();
  }
  return "";
}

function relevanceModeLabel(modeCode) {
  const m = Number(modeCode ?? 2);
  if (m <= 0) return "legacy_bbox";
  if (m === 1) return "relaxed_near_route";
  return "strict_directional";
}

function applyDirectionalRelevance(features = [], routePoints = [], runtimeSettings = {}) {
  const mode = clamp(Number(runtimeSettings.incident_relevance_mode ?? 2), 0, 2);
  const modeLabel = relevanceModeLabel(mode);
  const routeDir = routeDirectionToken(routePoints);
  const routeReady = Array.isArray(routePoints) && routePoints.length >= 2;
  const strictBuffer = Math.max(0.5, Number(runtimeSettings.incident_route_buffer_km_strict ?? 6));
  const relaxedBuffer = Math.max(strictBuffer, Number(runtimeSettings.incident_route_buffer_km_relaxed ?? 12));
  const bufferKm = mode >= 2 ? strictBuffer : relaxedBuffer;
  const relaxedDirPenalty = clamp(Number(runtimeSettings.incident_relaxed_direction_penalty ?? 0.45), 0, 1);

  let weightedCount = 0;
  let matched = 0;
  let distanceDropped = 0;
  let directionDropped = 0;
  let directionSoftPenalty = 0;

  for (const f of features || []) {
    const attrs = f?.attributes || {};
    let weight = Number(attrs.__weight ?? 1);
    if (!Number.isFinite(weight) || weight <= 0) weight = 1;

    if (mode > 0 && routeReady) {
      const pt = featurePoint(f);
      if (!pt) continue;
      const d = nearestRouteDistanceKm(pt, routePoints);
      if (!Number.isFinite(d) || d > bufferKm) {
        distanceDropped += 1;
        continue;
      }

      const itemDir = directionToken(featureDirectionText(attrs));
      if (routeDir && itemDir && itemDir !== routeDir) {
        if (mode >= 2) {
          directionDropped += 1;
          continue;
        }
        weight *= relaxedDirPenalty;
        directionSoftPenalty += 1;
      }
    }
  ...
  const count = Math.max(0, Math.round(weightedCount));
  return {
    mode_code: mode,
    mode_label: modeLabel,
    route_ready: routeReady,
    route_direction: routeDir || "",
    route_points: routePoints.length,
    buffer_km: mode > 0 ? bufferKm : null,
    raw_count: (features || []).length,
    matched_count: matched,
    weighted_count: Number(weightedCount.toFixed(2)),
    count,
    dropped_distance: distanceDropped,
    dropped_direction: directionDropped,
    relaxed_direction_penalty_applied: directionSoftPenalty
  };
}
...
export async function runCorridorPoll(corridor, { sampleLimit = 40, baselineAlpha = 0.2, runtimeSettings = {} } = {}) {
  ...
  const routePoints = orderedRoutePointsFromCheckpoints(checkpoints);

  const weatherPromise = fetchCorridorWeather(corridor, runtimeSettings, { useCache: true });
  const feedPromise = fetchNormalizedFeeds(corridor, {
    needTraffic: true,
    needIncidents: true,
    needClosures: true,
    needCameras: true,
  ...
  const [feedSnapshot, weather] = await Promise.all([feedPromise, weatherPromise]);

  const trafficFeatures = Array.isArray(feedSnapshot?.trafficFeatures) ? feedSnapshot.trafficFeatures : [];
  const incidentFeatures = Array.isArray(feedSnapshot?.incidentFeatures) ? feedSnapshot.incidentFeatures : [];
  const closureFeatures = Array.isArray(feedSnapshot?.closureFeatures) ? feedSnapshot.closureFeatures : [];
  const cameraFeatures = Array.isArray(feedSnapshot?.cameraFeatures) ? feedSnapshot.cameraFeatures : [];

  const validTraffic = trafficFeatures.filter((f) => {
    const a = f.attributes || {};
  ...
  const incidentRelevance = applyDirectionalRelevance(incidentFeatures, routePoints, runtimeSettings);
  const closureRelevance = applyDirectionalRelevance(closureFeatures, routePoints, runtimeSettings);
  const incidentsCount = Number(incidentRelevance.count || 0);
  const closuresCount = Number(closureRelevance.count || 0);

  const allCameraAttrs = cameraFeatures.map((f) => cameraRowFromFeature(f));
      ...
      [
        ...
        JSON.stringify({
          ...
          model: model
            ...
            : null,
          feedProfile: feedSnapshot.provider || corridor.feed_profile || "il_arcgis",
          relevance: {
            incidents: incidentRelevance,
            closures: closureRelevance
          },
          feedCounts: {
            traffic: trafficFeatures.length || 0,
            incidents_raw: incidentFeatures.length || 0,
            incidents_used: incidentsCount,
            closures_raw: closureFeatures.length || 0,
            closures_used: closuresCount,
            cameras: cameraFeatures.length || 0,
            ...
            provider_counts: feedSnapshot.counts || {}
          },
          feedMeta: feedSnapshot.meta || {},
          cameraSelection: {
