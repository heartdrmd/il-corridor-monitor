...
const NWS_RADAR_BASE_URL = "https://radar.weather.gov";
const NWS_RIDGE_STANDARD_PATH = "/ridge/standard";
const NWS_STATION_FRAME_COUNT = 10;
...
function normalizeRadarStationId(radarStation) {
  const raw = String(radarStation || "").trim().toUpperCase();
  if (!raw) return "";
...
function buildRadarStationAssets(radarStation) {
  const station = normalizeRadarStationId(radarStation);
  if (!station) {
    return {
      radar_station: "",
      radar_station_url: "",
      radar_station_loop_gif_url: "",
      radar_station_frames: [],
      radar_station_frame_count: 0,
      radar_station_source: ""
    };
  }
  const base = `${NWS_RADAR_BASE_URL}${NWS_RIDGE_STANDARD_PATH}`;
  const frames = Array.from({ length: NWS_STATION_FRAME_COUNT }, (_, i) => `${base}/${station}_${i}.gif`);
  return {
    radar_station: station,
    radar_station_url: buildRadarStationUrl(station),
    radar_station_loop_gif_url: `${base}/${station}_loop.gif`,
    radar_station_frames: frames,
    radar_station_frame_count: frames.length,
    radar_station_source: `${base}/${station}_*.gif`
  };
}
...
export async function fetchCorridorWeather(corridor, cfg = {}, opts = {}) {
  ...
  try {
    ...
    const out = {
      ...base,
      available: true,
      ...buildRadarStationAssets(radarStation),
