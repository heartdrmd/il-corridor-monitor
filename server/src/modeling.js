...
export function trainNextScoreModel(runs) {
...
export function predictNextScore(model, runLike) {
  if (!model) {
  ...
  const p50 = clamp(raw);
  const p90 = clamp(p50 + 1.2816 * model.sigma);

  const zScores = x.map((v, i) => Math.abs((v - model.featureStats.mu[i]) / (model.featureStats.sd[i] || 1)));
  const driftScore = mean(zScores.slice(1)) / 3.5;
  const confidence = clamp(100 - model.sigma * 2.2 - driftScore * 35, 5, 99);
